//Klebsiella risk factor model D
...
data{
  ...
  int<lower=1> N_month;                               //number of months in study (13)
  int<lower=1> month[N];                              //monthly index
...
model{
  ...
  beta ~ normal(0, 3);
  mu ~ normal(0, 3);                                 //hyperprior for sample mean
  sigma ~ normal(0, 1);                              //hyperprior for sample variance (half-normal)
  ...
  for (i in 1:N) {
    ...
    day_odds = exp(alpha[month[i]] + beta[1]*sex[i] + beta[2]*age_entry[i] + beta[3]*probio[i] + beta[4]*breast_milk[i] + beta[5]*severe[i] + beta[6]*premature[i] + beta[7]*e_coli[i] + beta[8]*nurses[i] + beta[9]*ampicillin_mono_96[i] + beta[10]*ampicillin_gentamicin_96[i] + beta[11]*ceftriaxone_mono_96[i] + beta[12]*cloxacillin_oral_96[i] + beta[13]*cloxacillin_gentamicin_96[i] + beta[14]*imipenem_mono_96[i]);    
